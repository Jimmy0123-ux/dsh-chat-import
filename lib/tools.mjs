// lib/tools.mjs — 31 个工具的注册（17 个 import_* + import_agents + doctor +
...
export function registerTools(ctx, registryDir) {
  ...
  // WorkBuddy 源：腾讯 AI 编码应用 WorkBuddy。每个会话一个 transcript
  // ~/.workbuddy/projects/<project-hash>/<session-uuid>.jsonl，逐行事件 JSON：
  // message（user/assistant + content block）/ reasoning / function_call /
  // function_call_result / file-history-snapshot。标准单文件/目录批量形态；
  // 文件名 stem（session-uuid）作稳定源 id 兜底（事件内 sessionId 优先）。
  ctx.tools.register(makeImportTool(ctx, {
    format: 'workbuddy',
    toolName: 'import_workbuddy',
    ...
    description:
      '从 WorkBuddy（腾讯 AI 编码应用）的会话 JSONL 导入历史对话为可继续的 DSH 会话（' +
      '~/.workbuddy/projects/<project-hash>/<session-uuid>.jsonl）。' +
      'path 可以是单个 .jsonl 文件，也可以是包含多个 .jsonl 的目录（目录模式递归扫描，每个文件导入为独立会话）。' +
      '解析 message（user 提问从 <user_query> 提取、assistant 文本）、reasoning→推理、' +
      'function_call / function_call_result 成对工具调用（按 callId 配对，孤儿/打断结果丢弃）并持久化；' +
      '重复导入同一会话会幂等跳过。返回新会话 id（或批量统计）与明细。',
  }))
  // REQ-59/61/64 外部 agent/mode prompt/skill/config -> DSH skills 资产（第 14 个工具）：
  // 非会话导入，独立注册。收集 pi/opencode/Claude/Codex 的自定义 agent / mode prompt /
