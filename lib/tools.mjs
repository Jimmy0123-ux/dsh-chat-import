// lib/tools.mjs — 19 个工具的注册（13 个 import_* + import_agents + export_claude
...
export function registerTools(ctx, registryDir) {
  ...
  ctx.tools.register(defineTool({
    name: 'import_agents',
    description:
      '把 pi（~/.pi/agent/{agents,prompts}）与 opencode（~/.config/opencode/{agents,skill}）的' +
  ...
  // REQ-36 反向同步（双向同步桥 B 第一步）：第 16 个工具，把 DSH 会话新增轮次
  ...
  // REQ-33 导入识别 / 撤回（只读）：第 17/18 个工具。平台无 delete 面
  ...
  // REQ-25/REQ-40 会话发现：第 19 个工具，只读扫描（发现核心在 lib/discovery.mjs，
