...
//            version, args, importedAt }；multi 用 conversations / sessions 子表
// 逐会话记录 { dshId, turns, events }。
//
...
async function decideItem(ctx, { known, converted, args, fingerprint, persisted, sourcePath }) {
...
export async function decideSingle(ctx, { known, converted, stat, args, fingerprint, persisted, sourcePath }) {
...
export async function decideMulti(ctx, { known, items, stat, args, fingerprint, persisted, sourcePath, subTable }) {
