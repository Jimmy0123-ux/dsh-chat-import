...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
        ...
        fetch("/api-import/sessions", {
          ...
          .then((data) => {
            ...
            if (data && data.ok === true) {
              ...
              setError((data && data.error) || "会话列表加载失败");
            }
          })
    ...
    /** 触发按钮：fixed 浮动胶囊（脱离 footer.action 行布局）。
     ...
     * footer occupant（cordis / 未来其它插件）都无法挡住；rail（wide=false）态只显图标。
     */
    function ImportButton({ wide }) {
      ...
      const pillStyle = {
        position: "fixed", left: "12px", bottom: "132px", zIndex: 10000,
        display: "flex", alignItems: "center", gap: "6px",
        background: colors.accent, color: "#ffffff", border: "none",
        cursor: "pointer", padding: rail ? "6px" : "6px 12px",
        borderRadius: "999px", fontSize: "12.5px", lineHeight: "1.4",
        boxShadow: "0 2px 10px rgba(0,0,0,.28)",
      };
      return React.createElement(React.Fragment, null,
        !open && React.createElement("button", {
          style: pillStyle, title: "从其他工具导入会话（发现 + 单选/多选导入）",
          "aria-label": "导入会话",
