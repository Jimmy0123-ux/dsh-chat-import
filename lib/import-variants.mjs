...
//   kimi      —— 源是会话目录（wire.jsonl + state.json + kimi.json workdir 映射）
...
import {
  loadImports, unwrapRecord, listPersistedIds,
...
// 递归收集会话目录：目录含 wire.jsonl 即会话（收下，不下钻——子代理 wire 不并入）；
// 否则 recursive 时下钻（sessions 根 → <hash>/ → <session-id>/ 两级结构）。
export async function collectKimiSessions(ctx, dirTarget, out, recursive) {
  const entries = await ctx.fs.listDir(dirTarget)
  for (const entry of entries) {
    if (entry.type !== 'directory') continue
    const sub = await ctx.fs.resolve(entry.target.displayPath || entry.target.targetKey)
    const wireTarget = await ctx.fs.resolve(join(sub.targetKey, 'wire.jsonl'))
    const wireStat = await ctx.fs.stat(wireTarget)
    if (wireStat && wireStat.type === 'file') {
      out.push(sub)
    } else if (recursive) {
...
// kimi 派生参数：kimiId（会话目录名，幂等源 id）、cwd（kimi.json md5 映射）、
// title（state.json custom_title，权威标题）。target 可以是会话目录或 wire.jsonl。
export async function kimiDeriveArgs(ctx, target) {
  ...
  const derived = {}
  const sessionDirName = isWireFile ? (segs[segs.length - 2] || '') : base
  if (sessionDirName) derived.kimiId = sessionDirName
  const hashDirName = isWireFile ? (segs[segs.length - 3] || '') : (segs[segs.length - 2] || '')
  const cwd = await kimiWorkDirByHash(ctx, String(p), hashDirName)
  if (cwd) derived.cwd = cwd
  // state.json 与 wire.jsonl 同目录（会话目录内）
  const sessionDirPath = isWireFile ? String(p).replace(/[\\/][^\\/]*$/, '') : String(p)
  const stateTarget = await ctx.fs.resolve(join(sessionDirPath, 'state.json'))
  try {
    const state = JSON.parse(await ctx.fs.readText(stateTarget))
    if (state && typeof state.custom_title === 'string' && state.custom_title.trim()) {
      derived.title = state.custom_title.trim()
    }
  } catch {
    // state.json 缺失/损坏不致命：标题回退首问
  }
  return derived
}
...
export async function importKimiSession(ctx, target, args, { registryDir, persisted } = {}) {
  ...
  const wireTarget = await ctx.fs.resolve(join(sourcePath, 'wire.jsonl'))
  ...
  const decision = await decideSingle(ctx, { known, converted: out, stat, args: merged, fingerprint, persisted: persistedSet, sourcePath, budget: merged.budget })
  return attachReq26(out, await runDecision(ctx, decision, registryDir, sourcePath, persistedSet))
}
