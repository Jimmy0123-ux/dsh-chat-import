...
`scan_discover` scans the known data roots of all 14 formats and returns a structured session index (title, project, cwd, path, import status, and git branch/dirty when the source directory is a git repo) so you can preview before a batch import. Zero side effects:
...
- **Fail loudly, never silently** — malformed lines and suspected secrets are counted and reported by position (line numbers / kind — content is never output); anything a source format cannot preserve is explicitly flagged in the import report. Every persisted session also runs a structural self-check (seq continuity, known event types, `surfaceOp` on surface events, valid `sourceEventSeqs`) — violations surface in the import result as a `validation` report.
