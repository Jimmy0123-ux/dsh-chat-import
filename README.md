...
- **11 sources, one plugin** — one tool per source, from Claude Code JSONL and Codex rollouts to SQLite databases and session directories.
...
**2. Import** — in any DSH session, import a single file or a whole directory (the same call shape works for all 11 import tools — see the table above):
...
`scan_discover` scans the known data roots of all 11 formats and returns a structured session index (title, project, path, import status) so you can preview before a batch import. Zero side effects:
...
Targets the `dsh 0.1.x` line (`dsh-tools ^0.1.0-rc.6`, tested on `dsh 0.1.0-rc.6`) and requires **Node.js >= 22.13** (the first release where `node:sqlite` is available without a flag). `npm test` — 324 cases.
