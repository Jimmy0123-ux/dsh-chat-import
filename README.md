...
- Returns `{ mode: 'single', sessionId, turns, messages, toolCalls, skipped, alreadyImported, status }` where `status` is `imported` | `already-imported` | `appended` | `skipped`, plus optional `appendedTurns` / `appendedEvents` (grew), `sourceShrunk` (truncated), `changedInPlace` (grew inside existing turns — append-only cannot rewrite them), `argsChanged` (import parameters changed), `budgetChanged` (context budget changed), `backfilled` (registry record restored for a legacy import), `forceImported: { previous, current }` (force / sessionId-change copy) and `droppedBoundaryResults`. Malformed lines, secrets and permissions ride along: `skippedLines` lists malformed records as `{ line, error }` (line numbers from 1 — the count is unlimited, the detail list is capped at 200), `secrets` lists suspected-secret locations as `{ line, kind }` (position only, content is never output), and `permissionCount` counts Claude-source permission records (0 omitted). In batch mode the same three fields appear per `results` entry.
...
- Optional `preview: true` (alias `dryRun: true`) runs the import **read-only** — the source is resolved, read and converted exactly like a real import, but nothing is persisted (no `create` / `append`, no imports-registry read or write, no workspace attach — zero side effects). It returns the same `mode` / `total` / `results` skeleton as a real import with a `preview: true` marker and no write-state fields; each entry carries the would-be session's `turns` / `messages` / `toolCalls` / `skipped` plus `title`, `cwd`, `createdAt`, and a `skipReason` when nothing is importable. Drop the flag and call again to actually import.
...
`scan_discover` is the read-only discovery side of the plugin: it scans the known data roots of all **11 formats** (Claude Code / Codex / ChatGPT CLI / Cursor / Gemini / Reasonix / opencode / ZCode / Grok Build / OpenClaw / Hermes, plus ChatGPT web exports) and returns a structured session index for previewing before a batch import:
...
- `path` (optional) — the scan root: a directory (e.g. `~/.claude/projects`) or a single file (e.g. a `.jsonl` transcript or `conversations.json`). Omitted, every format's default data root is scanned (ChatGPT has no automatic root — point `path` at its `conversations.json` explicitly).
