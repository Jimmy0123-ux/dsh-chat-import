...
test('apply 注册十九个工具（13 导入 + import_agents + scan_discover + export_claude + sync_to_claude + REQ-33 识别/撤回）', () => {
  const { ctx, registered } = makeCtx({})
  apply(ctx)
  assert.equal(registered.length, 19)
  const names = registered.map((d) => d.name).sort()
  assert.deepEqual(names, ['export_claude', 'import_agents', 'import_chatgpt', 'import_claude', 'import_codex', 'import_cursor', 'import_gemini', 'import_grokbuild', 'import_hermes', 'import_kimi', 'import_openclaw', 'import_opencode', 'import_pi', 'import_reasonix', 'import_zcode', 'list_imported_sessions', 'retract_import', 'scan_discover', 'sync_to_claude'])
...
test('REQ-41 apply 注册 webServer 路由（POST /api-import/sessions + /api-import/import，kind exact），工具计数不变', () => {
  ...
  // 只加路由，不加工具：13 导入 + import_agents + scan/export/sync/list/retract = 19，注册数不变
  assert.equal(registered.length, 19)
})
